...
::: warning 当前未发布正式版本
所有版本当前均处于 `0.x` 开发阶段，尚未发布任何正式（Minor）版本。

这意味着：接口契约、目录结构、配置项和 API 设计在版本间可能发生较大变动，不保证向后兼容性。如果你将本项目用作其他项目的起点，建议锁定到特定 commit 或 tag，而非直接跟踪 `main` 分支。待核心功能稳定、文档覆盖度达标后，将发布 v1.0.0；届时将提供明确的迁移指南和稳定的接口承诺。
:::
